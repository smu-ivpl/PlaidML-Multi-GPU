...
def _open_devices():
    global _ctx, _device_lock
    with _device_lock:
        dev = []
        devices = plaidml.devices(_ctx, return_all=True)
        for matched_device in devices[0]:
            if matched_device.id.find(b'opencl') == 0:
                dev.append(plaidml.Device(_ctx, matched_device))
        for unmatched_device in devices[1]:
            if unmatched_device.id.find(b'opencl') == 0:
                dev.append(plaidml.Device(_ctx, unmatched_device))
    return dev
...
class _Function(object):
    ...
    def __init__(self, inputs, outputs, updates, name, **kwargs):
        ...
        if kwargs.get('dev') != None:
            self._dev_IVPL = _device(kwargs['dev_no'])
        else:
            global IVPL_DEVICE_NO
            self._dev_IVPL = _device(IVPL_DEVICE_NO)
    ...
    def __call__(self, inputs):
        ...
        for (name, val) in zip(self._input_names, inputs):
            if isinstance(val, six.integer_types):
                ...
                val = variable(val, dtype=self._input_types[name], **{'dev': self._dev_IVPL}).var
        ...
        tensors = [
            plaidml.Tensor(self._dev_IVPL, self._invoker.get_output_shape(name))
            for name in self._output_names
        ]
...
def function(inputs, outputs, updates=None, name=None, **kwargs):
    ...
    if name == None:
        name = ''
    return _Function(inputs, outputs, updates, name, **kwargs)
...
def get_value(x):
    global IVPL_DEVICE_NO
    func = ptile.compose(_ctx, _device(IVPL_DEVICE_NO), [], [('out', x)], name='get_value')
    invoker = plaidml.Invoker(_ctx, func)
    shape = invoker.get_output_shape('out')
    tensor = plaidml.Tensor(_device(IVPL_DEVICE_NO), shape)
...
def variable(value, dtype=None, name=None, constraint=None, **kwargs):
    dtype = dtype or floatx()
    if constraint:
    ...
    if kwargs.get('dev') != None:
        ...
        dev = _device(IVPL_DEVICE_NO)
    if isinstance(value, float) or isinstance(value, six.integer_types):
        tensor = plaidml.Tensor(dev, plaidml.Shape(_ctx,
        ...
        func = ptile.compose(_ctx, dev, [], [('out', value)], name='variable')
        invoker = plaidml.Invoker(_ctx, func)
        shape = invoker.get_output_shape('out')
        tensor = plaidml.Tensor(dev, shape)
        invoker.set_output('out', tensor)
        invoker.invoke()
    ...
    tensor = plaidml.Tensor(
        dev, plaidml.Shape(_ctx, ptile.convert_np_dtype_to_pml(dtype), *value.shape))
